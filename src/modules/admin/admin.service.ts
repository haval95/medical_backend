...
import {
  ...
  createScheduleTemplateForDoctor,
  listDoctorSlots,
  updateDoctorLocationById,
...
const buildPaginatedResult = <T>(items: T[], page: number, pageSize: number, totalItems: number) => {
...
type WorkingHourDay = (typeof workingHourDays)[number];

type GeneralWorkingHour = {
  day: WorkingHourDay;
  isActive: boolean;
  startTime?: string;
  endTime?: string;
...
const normalizeGeneralWorkingHours = (
  value: Prisma.JsonValue | null | undefined
): GeneralWorkingHour[] => {
  const defaults = new Map<WorkingHourDay, GeneralWorkingHour>(
    workingHourDays.map((day) => [day, { day, isActive: false }])
  );
  ...
  if (Array.isArray(value)) {
    for (const entry of value) {
      ...
      if (typeof day !== 'string' || !workingHourDays.includes(day as WorkingHourDay)) {
        continue;
      }

      const startTime =
        typeof candidate.startTime === 'string' && timePattern.test(candidate.startTime)
          ? candidate.startTime
          : undefined;
      const endTime =
        typeof candidate.endTime === 'string' && timePattern.test(candidate.endTime)
          ? candidate.endTime
          : undefined;
      const isActive = Boolean(candidate.isActive) && Boolean(startTime && endTime);

      defaults.set(day as WorkingHourDay, {
        day: day as WorkingHourDay,
        isActive,
...
const serializeGeneralWorkingHours = (
  value?: GeneralWorkingHour[]
...
const serializeVisitDetailJson = (
...
const mapAdminDoctor = (
  doctor: Prisma.DoctorProfileGetPayload<{ include: typeof adminDoctorListInclude }>
) => ({
  id: doctor.id,
  ...
  generalWorkingHours: normalizeGeneralWorkingHours(
    doctor.generalWorkingHours as Prisma.JsonValue | null | undefined
  ),
  _count: {
...
const mapAdminPatient = (
  patient: Prisma.PatientProfileGetPayload<{ include: typeof adminPatientInclude }>
) => ({
  id: patient.id,
...
const mapAdminRequest = (
  request: Prisma.ServiceRequestGetPayload<{ include: typeof adminRequestInclude }>
) => ({
  id: request.id,
  type: request.type,
...
const mapOnboardingStep = (
  step: Prisma.OnboardingStepGetPayload<Record<string, never>>
) => ({
  id: step.id,
  title: step.title,
...
const mapAdminUser = (
  user: Prisma.UserGetPayload<{ include: typeof adminUserInclude }>
) => ({
  id: user.id,
  ...
  doctorProfile: user.doctorProfile
    ? {
        ...
        generalWorkingHours: normalizeGeneralWorkingHours(
          user.doctorProfile.generalWorkingHours as Prisma.JsonValue | null | undefined
        ),
...
export const listAdminRequestDirectory = async (filters: {
  ...
  type?: 'SPECIFIC_DOCTOR' | 'ANY_AVAILABLE_DOCTOR';
  city?: string;
  sortBy?: 'NEWEST' | 'OLDEST' | 'CITY';
  page: number;
  pageSize: number;
}) => {
...
export const getAdminRequestById = async (requestId: string) => {
...
export const createAdminManagedUser = async (input: {
  ...
  doctorProfile?: {
    ...
    generalWorkingHours?: GeneralWorkingHour[];
    isAvailable?: boolean;
  ...
  return prisma.$transaction(async (tx) => {
    let referredByDoctorId: string | undefined;
    ...
    const created = await tx.user.create({
      data: {
        ...
        doctorProfile:
          input.role === Role.DOCTOR && input.doctorProfile
            ? {
                create: {
                  referralCode:
                    input.doctorProfile.referralCode ?? `DOC-${phoneNumber.slice(-6)}`.toUpperCase(),
                  ...
                  generalWorkingHours: serializeGeneralWorkingHours(
                    input.doctorProfile.generalWorkingHours
                  ),
          ...
          input.role === Role.PATIENT
            ? {
                create: {
                  ...
                  lifetimePoints:
                    input.patientProfile?.lifetimePoints ??
...
export const updateAdminManagedUser = async (
  ...
  const updated = await prisma.user.update({
    ...
    data: {
      ...
      photoUrl: input.photoUrl,
      permissions:
        input.permissions && (input.role ?? existing.role) === Role.ADMIN
          ? resolvePermissions(Role.ADMIN, input.permissions)
          : undefined,
    },
    include: adminUserInclude,
...
export const listAdminDoctorDirectory = async (filters: {
  ...
  city?: string;
  sortBy?: 'NAME' | 'RATING' | 'VISITS' | 'RADIUS';
  page: number;
  pageSize: number;
}) => {
  ...
  const primaryOrderBy: Prisma.DoctorProfileOrderByWithRelationInput =
...
export const getAdminDoctorById = async (doctorId: string) => {
...
export const deleteOnboardingStep = async (stepId: string) => {
  const existing = await prisma.onboardingStep.findUnique({
    where: {
      id: stepId,
    },
...
export const uploadOnboardingImage = async (input: {
  fileName: string;
  mimeType: string;
  contentBase64: string;
}) =>
...
export const listAdminPatientDirectory = async (filters: {
  ...
  const primaryOrderBy: Prisma.PatientProfileOrderByWithRelationInput =
...
export const getAdminPatientById = async (patientProfileId: string) => {
...
export const updateAdminPatient = async (
  patientProfileId: string,
  input: {
    fullName?: string;
    city?: string;
...
export const createAdminDiscount = async (input: {
  ...
  title: string;
  description?: string;
  type: Prisma.DiscountUncheckedCreateInput['type'];
  value: number;
  pointsCost?: number;
...
export const adjustAdminPoints = async (
  patientProfileId: string,
  points: number,
  notes: string
) => grantAdminAdjustment(patientProfileId, points, notes);
...
export const listAdminReviewDirectory = async (filters: {
  ...
  const orderBy: Prisma.ReviewOrderByWithRelationInput[] =
    filters.sortBy === 'RATING' ? [{ rating: 'desc' }, { createdAt: 'desc' }] : [{ createdAt: 'desc' }];
...
const buildDateTimeRange = (from?: string, to?: string) =>
  from || to
    ? {
...
const buildAdminReportFilters = (filters: AdminReportFilters) => {
...
export const getAdminReportOverview = async (filters: AdminReportFilters) => {
...
export const getAdminReportTable = async (
  filters: AdminReportFilters & {
    tab: AdminReportTableTab;
    page: number;
    pageSize: number;
  }
) => {
  ...
  if (filters.tab === 'topDoctors') {
    ...
    const completedCountMap = new Map(
      completedCounts.map((item) => [item.doctorId, item._count._all])
    );
    ...
    const pageItems = rankedDoctors.slice(
  ...
  if (filters.tab === 'cityDemand') {
    ...
    return {
      ...
      ...buildPaginatedResult(
        rankedCities.slice((filters.page - 1) * filters.pageSize, filters.page * filters.pageSize),
        filters.page,
        filters.pageSize,
        rankedCities.length
      ),
  ...
  if (filters.tab === 'appointmentStatus') {
    ...
    return {
      ...
      ...buildPaginatedResult(
        rows.slice((filters.page - 1) * filters.pageSize, filters.page * filters.pageSize),
        filters.page,
        filters.pageSize,
        rows.length
      ),
...
const resolveRetentionDelete = async (
  tx: PrismaExecutor,
  appliesTo: string,
  cutoff: Date,
  dryRun: boolean
  ...
  switch (normalized) {
    ...
    default:
      throw new ApiError(
        400,
        `Unsupported retention target "${appliesTo}". Use AUDIT_LOG, NOTIFICATION, AUTH_CHALLENGE, BACKUP_OPERATION, or DATA_GOVERNANCE_REQUEST.`
      );
  }
};

const buildGovernanceExportData = async (
  tx: PrismaExecutor,
  request: Prisma.DataGovernanceRequestGetPayload<{
    include: {
...
const executeGovernanceDeletion = async (
  tx: PrismaExecutor,
  request: Prisma.DataGovernanceRequestGetPayload<{
    include: {
      subjectPatientProfile: {
...
export const listRetentionRules = async () =>
  prisma.documentRetentionRule.findMany({
    orderBy: {
      createdAt: 'desc',
    },
  });

export const createRetentionRule = async (input: {
  name: string;
  description?: string;
  appliesTo: string;
  retainDays: number;
  ...
  prisma.documentRetentionRule.create({
    data: {
      name: input.name,
      description: input.description,
...
export const runRetentionRule = async (ruleId: string, input?: { dryRun?: boolean }) => {
...
export const listSecurityIncidents = async () =>
...
export const updateSecurityIncident = async (
  ...
  prisma.securityIncident.update({
    ...
    data: {
      ...
      remediationNotes: input.remediationNotes,
      resolvedAt:
        input.status === SecurityIncidentStatus.RESOLVED ||
...
export const updateAccessReview = async (
  reviewId: string,
  adminUserId: string,
  input: {
...
export const listDataGovernanceRequests = async () =>
  prisma.dataGovernanceRequest.findMany({
    include: {
      subjectPatientProfile: {
        include: {
          user: {
...
export const createDataGovernanceRequest = async (
  adminUserId: string,
  input: {
  ...
  prisma.dataGovernanceRequest.create({
    data: {
...
export const processDataGovernanceRequest = async (
  requestId: string,
  adminUserId: string,
  input: {
    status?: GovernanceRequestStatus;
  ...
  return prisma.$transaction(async (tx) => {
    let resultData: Prisma.JsonObject | undefined;
    ...
    return tx.dataGovernanceRequest.update({
      ...
      data: {
        ...
        resultData,
        completedAt:
          input.executeAction || input.status === GovernanceRequestStatus.COMPLETED
            ? new Date()
            : undefined,
      },
      include: {
...
export const listBackupOperations = async () =>
...
export const createBackupOperation = async (
  adminUserId: string,
  input: {
...
export const updateBackupOperation = async (
  operationId: string,
  input: {
    status?: BackupOperationStatus;
    provider?: string;
...
export const getAdminNotificationQueue = async (limit?: number) =>
  getNotificationQueueOverview(limit);

export const processAdminNotificationQueue = async (limit?: number) =>
  processQueuedNotificationDeliveries(limit ?? 50);

export const createAdminDoctorScheduleTemplate = async (
  doctorId: string,
  input: Parameters<typeof createScheduleTemplateForDoctor>[1]
) => createScheduleTemplateForDoctor(doctorId, input);

export const createAdminDoctorUnavailability = async (
  doctorId: string,
  input: Parameters<typeof createDoctorUnavailability>[1]
) => createDoctorUnavailability(doctorId, input);

export const getAdminDoctorSlots = async (doctorId: string, from?: string, to?: string) =>
  listDoctorSlots(doctorId, from, to);
