...
const getModelDelegate = (model: string) =>
...
const readBeforeSnapshot = async (model: string, where?: Record<string, unknown>) => {
  if (!where) {
    return undefined;
  }
  ...
  query: {
    $allModels: {
      async $allOperations({ model, operation, args, query }) {
        ...
        const before =
          operation === 'update' || operation === 'upsert' || operation === 'delete'
...
export type PrismaExecutor = Omit<
  typeof prisma,
