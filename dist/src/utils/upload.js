...
const ensureSpacesConfigured = () => {
    if (!env.DO_SPACES_ACCESS_KEY || !env.DO_SPACES_SECRET_KEY) {
        throw new ApiError(500, 'DigitalOcean Spaces credentials are not configured on the backend environment');
    }
};
...
export const getManagedUploadKeyFromUrl = (fileUrl) => {
    if (!fileUrl) {
        return null;
    }
...
export const deleteFileFromSpaces = async (key) => {
    const client = createSpacesClient();
