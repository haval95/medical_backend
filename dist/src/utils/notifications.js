...
const createDeliveryPayload = (delivery) => ({
    to: delivery.token,
...
const sendExpoPushBatch = async (deliveries) => {
...
const queueNotificationDeliveries = async (notificationId, userId) => {
    ...
    return Promise.all(activeTokens.map((entry) => prisma.notificationDelivery.create({
        data: {
            notificationId,
            token: entry.token,
...
export const processQueuedNotificationDeliveries = async (limit = 50) => {
    ...
    await Promise.all(deliveries.map((delivery) => prisma.notificationDelivery.update({
    ...
    const providerGroups = deliveries.reduce((groups, delivery) => {
    ...
    for (const [provider, providerDeliveries] of Object.entries(providerGroups)) {
        try {
            ...
            await Promise.all(providerDeliveries.map((delivery) => prisma.notificationDelivery.update({
                where: { id: delivery.id },
                data: {
            ...
            const failureReason = error instanceof Error ? error.message : 'Push delivery failed unexpectedly';
            await Promise.all(providerDeliveries.map((delivery) => prisma.notificationDelivery.update({
                where: { id: delivery.id },
                data: {
...
export const createNotifications = async (inputs) => Promise.all(inputs.map((input) => createNotification(input)));
export const registerPushToken = async (userId, input) => prisma.pushToken.upsert({
...
export const listMyNotifications = async (userId) => prisma.notification.findMany({
    where: { userId },
    orderBy: { createdAt: 'desc' },
    take: 100,
});
export const markNotificationRead = async (userId, notificationId) => prisma.notification.updateMany({
    where: {
        id: notificationId,
        userId,
    },
