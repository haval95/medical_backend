import { AppointmentStatus, NotificationType, Role, ScheduleSlotStatus, ServiceRequestStatus, ServiceRequestType, UserStatus, } from '@prisma/client';
...
const selectAdminRecipients = async () => prisma.user.findMany({
    where: {
        role: Role.ADMIN,
...
const releaseRejectedAppointmentAllocation = async (tx, appointmentId, slotId) => {
    if (!slotId) {
        return;
    }
...
export const createServiceRequest = async (userId, input) => {
    ...
    const notificationBatch = admins.map((admin) => ({
        userId: admin.id,
...
export const getRequestsForUser = async (userId, role) => {
...
export const updateServiceRequestStatus = async (userId, role, requestId, status) => {
    ...
    if (role === Role.DOCTOR) {
        ...
        const belongsToDoctor = request.assignedDoctorId === doctorProfile.id || request.requestedDoctorId === doctorProfile.id;
        if (!belongsToDoctor) {
...
export const acceptServiceRequest = async (userId, requestId) => {
    const doctorProfile = await prisma.doctorProfile.findUnique({
        where: { userId },
    });
    if (!doctorProfile) {
...
export const rejectServiceRequest = async (userId, requestId, reason) => {
    const doctorProfile = await prisma.doctorProfile.findUnique({
        where: { userId },
    });
    if (!doctorProfile) {
...
export const assignDoctorToRequest = async (requestId, doctorId) => {
    ...
    const updated = await prisma.$transaction(async (tx) => {
        await tx.serviceRequest.update({
            ...
            data: {
                ...
                status: ServiceRequestStatus.ASSIGNED,
                distanceKm: toDecimalInput(distanceKm),
                isWithinDoctorRange: distanceKm === null
